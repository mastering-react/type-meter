...
>(({ className, children, chevronDown = true, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={twMerge(
      "flex w-full  items-center justify-between whitespace-nowrap rounded-md border border-dark-300 bg-transparent  px-4 py-2 text-sm text-dark-800 placeholder:text-dark-300 focus:border-dark-500 focus:outline-none focus:ring-0 disabled:cursor-not-allowed disabled:opacity-50",
      className
    )}
    {...props}
  >
...
>(({ className, children, showCheck = true, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={twMerge(
      "relative flex w-full cursor-pointer hover:bg-slate-100 transition duration-150 items-center rounded-md py-2 px-4 pe-2 text-sm text-dark-800 outline-none focus:bg-dark-100 focus:text-dark-900 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
